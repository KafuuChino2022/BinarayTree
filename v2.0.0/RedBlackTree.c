...
static void* getRBTreeRoot(void* root){
    return (void*)(((RBTree)root) -> root);
}

/* 红黑树获取树大小 */
static int getRBTreeSize(void *root){
    return ((RBTree)root) -> size;
}

/* 红黑树获取子结点 */
static void* getRBTreeChild(void* node, int child){
    return (child == RIGHT) ? (void*)(((RBNode)node) -> rightChild) : (void*)(((RBNode)node) -> leftChild);
}
...
/* 红黑树获取父结点 */
static void* getRBTreeParent(void* node){
    return (void*)(((RBNode)node) -> parent);
}

/* 重设红黑树子结点 */
static void setRBTreeChild(void* node, void* value, int child){
    if (child == LEFT){
        ((RBNode)node) -> leftChild = ((RBNode)value);
    }else{
        ((RBNode)node) -> rightChild = ((RBNode)value);
    }
}

/* 重设红黑树父结点 */
static void setRBTreeParent(void* node, void* value){
    ((RBNode)node) -> parent = ((RBNode)value);
}

/* 重设红黑树根结点 */
static void setRBTreeRoot(void* root, void* value){
    ((RBTree)root) -> root = ((RBNode)value);
}
...
{
    return rotation(root, sign, LEFT, getRBTreeChild, getRBTreeParent, setRBTreeChild, setRBTreeParent, setRBTreeRoot);
}

/* 对sign右旋 */
Status rightRotationRB(RBTree root, RBNode sign)
{
    return rotation(root, sign, RIGHT, getRBTreeChild, getRBTreeParent, setRBTreeChild, setRBTreeParent, setRBTreeRoot);
}
